...
var mtCookieDomain = ".ruanyifeng.com";
var mtCookiePath = "/";
...
(function () {
  ...
  function _initializeVariables() {
    ...
    loadingIcon = "<img title='Loading...' src='/static/images/indicator.white.gif' alt='Loading' />";
  ...
  function _updateComments() {
    ...
    jsonUrl = [
        "http://www.ruanyifeng.com/cgi-bin/mtos/mt-comments.cgi?__mode=comment_listing&direction=",
        direction,
        "&entry_id=",
        entryID,
        "&limit=",
...
function mtFetchUser(cb) {
    ...
    if ( ( cb == 'mtSetUser' ) && mtGetUser() ) {
        ...
        script.src = 'http://www.ruanyifeng.com/cgi-bin/mtos/mt-comments.cgi?__mode=session_js&blog_id=2&jsonp=' + cb + '&ts=' + ts;
...
function mtCommentOnSubmit(f) {
    if (!mtRequestSubmitted) {
        ...
        if (f.armor)
            f.armor.value = 'c3c0bf429f4b1411a954f699e62dcddb04c9a452';
...
function mtSignIn() {
    ...
    var url = 'http://www.ruanyifeng.com/cgi-bin/mtos/mt-comments.cgi?__mode=login&blog_id=2';
    if (is_preview) {
        if ( document['comments_form'] ) {
            var entry_id = document['comments_form'].entry_id.value;
            url += '&entry_id=' + entry_id;
        } else {
            url += '&return_url=http%3A%2F%2Fwww.ruanyifeng.com%2F';
        }
    } else {
...
function mtSignOut(entry_id) {
    ...
    var doc_url = document.URL;
    doc_url = doc_url.replace(/#.+/, '');
    var url = 'http://www.ruanyifeng.com/cgi-bin/mtos/mt-comments.cgi?__mode=handle_sign_in&static=0&logout=1';
    if (is_preview) {
        if ( document['comments_form'] ) {
            var entry_id = document['comments_form'].entry_id.value;
            url += '&entry_id=' + entry_id;
...
function mtShowGreeting() {
    ...
    if ( u && u.is_authenticated ) {
        if ( u.is_banned ) {
            ...
            var user_link;
            if ( u.is_author ) {
                user_link = '<a href="http://www.ruanyifeng.com/cgi-bin/mtos/mt-comments.cgi?__mode=edit_profile&return_url=' + encodeURIComponent( location.href );
                user_link += '">' + u.name + '</a>';
            } else {
